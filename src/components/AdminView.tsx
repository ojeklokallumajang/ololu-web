...
export default function AdminView() {
  ...
  if (!isSuperUser) return <div className="flex flex-col items-center justify-center min-h-screen p-8 text-center bg-white"><ShieldAlert size={64} className="text-red-500 mb-4" /><h2 className="text-2xl font-black text-gray-800">Akses Terbatas</h2><p className="text-sm text-gray-500 mt-2 leading-relaxed">Halaman ini hanya untuk Administrator Utama PT Ololu Pengantaran Nusantara Lumajang.</p></div>;
    ...
    <div className="max-w-md mx-auto bg-[#FAFBF9] min-h-screen pb-20 relative font-sans text-left">
      <div className="bg-[#034F2A] text-white p-5 border-b-2 border-[#D4AF37] relative overflow-hidden">
        <div className="flex items-center justify-between">
          ...
          {pendingVerifList.length > 0 && <button onClick={() => setActiveTab('dompet')} className="bg-[#B8941F] text-white text-[8px] font-black px-3 py-1.5 rounded-full animate-bounce shadow-lg flex items-center space-x-1 tracking-tighter"><Bell size={10} /><span>{pendingVerifList.length} ANTRIAN</span></button>}
        </div>
      ...
      <div className="p-4 space-y-4">
          ...
          <div className="space-y-4 animate-in fade-in duration-500">
            ...
            <div className="bg-white p-4 rounded-3xl border border-gray-150 shadow-sm space-y-3">
               <div className="flex justify-between items-center px-1">
                 ...
                 <span className="text-[9px] font-black text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full animate-pulse uppercase">Live monitoring</span>
               </div>
               ...
               <div className="bg-emerald-50 p-2.5 rounded-xl flex items-center justify-center space-x-4 border border-emerald-100">
                  <div className="flex items-center space-x-1.5"><div className="w-2.5 h-2.5 bg-emerald-500 rounded-full animate-pulse"></div><span className="text-[10px] font-black text-emerald-800 uppercase tracking-widest">{sopirList.filter(s=>s.statusOnline).length} ONLINE</span></div>
                  <div className="flex items-center space-x-1.5"><div className="w-2.5 h-2.5 bg-blue-500 rounded-full"></div><span className="text-[10px] font-black text-blue-800 uppercase tracking-widest">{activeTrips.length} TRIP AKTIF</span></div>
               </div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'sopir' && (
          <div className="space-y-4 animate-in slide-in-from-bottom-2 duration-300">
             <div className="relative"><input type="text" placeholder="Cari Nama / Plat Rider..." value={searchDriverQuery} onChange={(e) => setSearchDriverQuery(e.target.value)} className="w-full p-4 pl-12 bg-white border rounded-2xl text-xs font-bold outline-none focus:border-[#046A38] transition-all shadow-sm" /><Search size={20} className="absolute left-4 top-3.5 text-gray-400" /></div>
             <div className="space-y-2">
                <div className="flex justify-between items-center px-1"><h3 className="text-[10px] font-black text-amber-600 uppercase tracking-widest">⏳ Antrian Verifikasi Berkas</h3><button onClick={syncData} className="p-1 text-gray-400 hover:text-amber-600"><Radio size={14} className={loading ? 'animate-spin' : ''} /></button></div>
                {sopirList.filter(s => !s.disetujuiAdmin && !s.ditolakAdmin && s.platNomor).length === 0 ? <p className="text-[10px] italic text-gray-400 bg-white p-6 rounded-3xl border border-dashed text-center">Belum ada pendaftaran baru.</p> :
                  sopirList.filter(s => !s.disetujuiAdmin && !s.ditolakAdmin && s.platNomor).map(s => (
                    <button key={s.id} onClick={() => { setSelectedSopir(s); setShowSopirModal(true); }} className="w-full bg-white p-4 rounded-2xl border-2 border-amber-100 flex items-center justify-between shadow-sm hover:border-amber-400 transition-all text-left"><div className="flex items-center space-x-4"><div className="w-12 h-12 bg-amber-50 rounded-full flex items-center justify-center text-amber-600 shadow-inner"><User size={24} /></div><div><p className="text-sm font-black text-gray-800 uppercase leading-none">{(s as any).nama || 'MITRA BARU'}</p><p className="text-[10px] text-amber-600 font-bold mt-1">Cek Berkas {s.platNomor}</p></div></div><ChevronRight size={20} className="text-amber-300" /></button>
                  ))
                }
             </div>
                ...
                <div className="space-y-3">{transaksiList.filter(t => t.statusTarik === 'menunggu').map(t => (
                  <div key={t.id} className={`bg-white p-5 rounded-3xl border-2 shadow-lg space-y-4 ${t.jenis === 'topup' ? 'border-emerald-500' : 'border-[#D4AF37]'}`}>
                    ...
                    <div className="bg-gray-50 p-4 rounded-2xl border text-center relative overflow-hidden">
                       ...
                       <span className="text-[10px] font-bold text-gray-400 uppercase block tracking-widest mb-1">Nominal Transaksi:</span>
           ...
           <div className="space-y-3 animate-in fade-in duration-300">
             ...
             <div className="space-y-2">
               {profilList.filter(p => p.peran === 'penumpang' && (p.nama?.toLowerCase().includes(searchUserQuery.toLowerCase()) || p.nomorHp?.toLowerCase().includes(searchUserQuery.toLowerCase()))).map(p => (
                   <div key={p.id} className="bg-white p-4 rounded-2xl border border-gray-150 flex items-center justify-between shadow-xs cursor-pointer active:scale-[0.98] transition-all" onClick={() => setSelectedUserDetail(p)}>
        ...
        {activeTab === 'pesanan' && (
           <div className="space-y-3 animate-in fade-in duration-300">
             <h3 className="text-[10px] font-black text-gray-700 uppercase tracking-widest px-1">Audit Order Masuk</h3>
        ...
        {activeTab === 'darurat' && (
           <div className="space-y-4 animate-in fade-in duration-300">
             <h3 className="text-xs font-black text-red-600 uppercase px-1 flex items-center space-x-2"><AlertTriangle size={16} /> <span>Pusat Kendali Darurat (SOS)</span></h3>
               ...
               emergencyList.map(e => (
                 <div key={e.id} className={`bg-white p-5 rounded-[32px] border-2 flex flex-col space-y-4 shadow-xl ${e.status === 'baru' ? 'border-red-600 animate-pulse' : 'border-gray-100'}`}>
                    ...
                    <div className="flex space-x-3">
                       <a href={`tel:${e.nomorHpPelapor}`} className="flex-1 py-4 bg-emerald-600 text-white text-center rounded-[20px] text-[11px] font-black uppercase shadow-lg">Hubungi</a>
                       <button className="flex-1 py-4 bg-gray-900 text-white text-center rounded-[20px] text-[11px] font-black uppercase shadow-lg">Selesai</button>
                    </div>
                 </div>
               ))
             }
            ...
            <div className="space-y-10 pb-32">
               ...
               <div className="space-y-4">
                  ...
                  <div className="bg-white p-6 rounded-[32px] border shadow-sm grid grid-cols-2 gap-4">
                     ...
                     <div className="space-y-1"><label className="text-[9px] font-bold text-gray-400 uppercase ml-1">Min. Tarif (Rp)</label><input type="number" value={tempConfig.ojekTarifMinimum} onChange={(e)=>setTempConfig({...tempConfig, ojekTarifMinimum: parseInt(e.target.value)})} className="w-full p-3.5 bg-gray-50 border rounded-2xl outline-none text-sm font-black" /></div>
               ...
               {/* CATEGORY: FOOD */}
               <div className="space-y-4">
                  <h3 className="text-[11px] font-black text-amber-700 uppercase tracking-[0.2em] border-l-4 border-amber-500 pl-3 flex items-center space-x-2"><ShoppingBag size={18} /> <span>Antar Makan & Belanja</span></h3>
                  <div className="bg-white p-6 rounded-[32px] border shadow-sm grid grid-cols-2 gap-4">
                     <div className="space-y-1"><label className="text-[9px] font-bold text-gray-400 uppercase ml-1">Tarif Dasar (Rp)</label><input type="number" value={tempConfig.makananTarifDasar} onChange={(e)=>setTempConfig({...tempConfig, makananTarifDasar: parseInt(e.target.value)})} className="w-full p-3.5 bg-gray-50 border rounded-2xl outline-none text-sm font-black" /></div>
                     <div className="space-y-1"><label className="text-[9px] font-bold text-gray-400 uppercase ml-1">Per KM (Rp)</label><input type="number" value={tempConfig.makananTarifPerKm} onChange={(e)=>setTempConfig({...tempConfig, makananTarifPerKm: parseInt(e.target.value)})} className="w-full p-3.5 bg-gray-50 border rounded-2xl outline-none text-sm font-black" /></div>
                     <div className="space-y-1"><label className="text-[9px] font-bold text-gray-400 uppercase ml-1">Mampir Toko (Rp)</label><input type="number" value={tempConfig.makananBiayaPerStop} onChange={(e)=>setTempConfig({...tempConfig, makananBiayaPerStop: parseInt(e.target.value)})} className="w-full p-3.5 bg-gray-50 border rounded-2xl outline-none text-sm font-black text-amber-600" /></div>
                     <div className="space-y-1 col-span-2 pt-2 border-t border-dashed mt-2"><label className="text-[9px] font-black text-amber-600 uppercase ml-1">Komisi Aplikasi (%)</label><input type="number" value={tempConfig.makananPersenJasa} onChange={(e)=>setTempConfig({...tempConfig, makananPersenJasa: parseInt(e.target.value)})} className="w-full p-4 bg-amber-50 border-2 border-amber-100 rounded-3xl outline-none text-lg font-black text-amber-700 shadow-inner" /></div>
                  </div>
               </div>

               {/* CATEGORY: SYSTEM & KEYS */}
               <div className="space-y-4">
                  ...
                  <div className="bg-white p-6 rounded-[32px] border shadow-sm space-y-6">
                     <div className="bg-slate-900 text-white p-5 rounded-3xl space-y-4 shadow-2xl">
                        ...
                        <div className="flex justify-between items-end"><div><div className="text-4xl font-black">{config.googleApiUsageCount?.toLocaleString() || 0}</div><div className="text-[9px] text-slate-500 font-bold uppercase tracking-widest mt-1">Total Panggilan API</div></div><div className="text-right"><div className="text-xl font-black text-emerald-400">{tempConfig.googleApiLimit?.toLocaleString() || 25000}</div><div className="text-[9px] text-slate-500 font-bold uppercase tracking-widest mt-1">Limit (Kuota)</div></div></div>
                        <div className="w-full h-3 bg-slate-800 rounded-full overflow-hidden border border-slate-700"><div className={`h-full transition-all duration-1000 ease-out ${ (config.googleApiUsageCount / config.googleApiLimit) > 0.9 ? 'bg-red-500 shadow-[0_0_10px_#ef4444]' : 'bg-gradient-to-r from-emerald-500 to-emerald-300' }`} style={{ width: `${Math.min(100, (config.googleApiUsageCount / config.googleApiLimit) * 100)}%` }}></div></div>
                     </div>
                     <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1"><label className="text-[9px] font-black text-gray-400 uppercase ml-1">Limit Panggilan</label><input type="number" value={tempConfig.googleApiLimit} onChange={(e)=>setTempConfig({...tempConfig, googleApiLimit: parseInt(e.target.value)})} className="w-full p-3.5 bg-gray-50 border-2 border-transparent focus:border-[#046A38] rounded-2xl outline-none text-sm font-black text-[#046A38]" /></div>
                        <div className="space-y-1"><label className="text-[9px] font-black text-gray-400 uppercase ml-1">Radius Cari (KM)</label><input type="number" value={tempConfig.radiusPencarianSopirKm} onChange={(e)=>setTempConfig({...tempConfig, radiusPencarianSopirKm: parseInt(e.target.value)})} className="w-full p-3.5 bg-gray-50 border rounded-2xl outline-none text-sm font-black" /></div>
                     </div>
                     {isSuperUser && (
                       <div className="space-y-4 pt-6 border-t-2 border-red-100 mt-2">
                          ...
                          <div className="space-y-3">
                             <div className="space-y-1"><label className="text-[8px] font-black text-gray-400 uppercase ml-1">Fonnte WA Token</label><div className="relative"><input type="password" value={tempConfig.fonnteToken} onChange={(e)=>setTempConfig({...tempConfig, fonnteToken: e.target.value})} className="w-full p-4 bg-red-50/30 border-2 border-dashed border-red-100 rounded-3xl outline-none text-xs font-mono focus:border-red-400 transition-all" /><KeyRound size={16} className="absolute right-4 top-3.5 text-red-200" /></div></div>
                             <div className="space-y-1"><label className="text-[8px] font-black text-gray-400 uppercase ml-1">Google Maps Key</label><div className="relative"><input type="password" value={tempConfig.googleMapsKey} onChange={(e)=>setTempConfig({...tempConfig, googleMapsKey: e.target.value})} className="w-full p-4 bg-red-50/30 border-2 border-dashed border-red-100 rounded-3xl outline-none text-xs font-mono focus:border-red-400 transition-all" /><MapPin size={16} className="absolute right-4 top-3.5 text-red-200" /></div></div>
                          </div>
                       </div>
                     )}
                  </div>
               </div>
               <button onClick={saveConfigWithLog} className="w-full py-6 bg-[#046A38] text-white font-black rounded-[40px] text-sm tracking-[0.3em] uppercase shadow-2xl active:scale-95 transition-all sticky bottom-6 z-40 border-b-8 border-emerald-900 shadow-emerald-900/40">💾 Simpan Perubahan Global</button>
            </div>
          </div>
        )}
        ...
        {activeTab === 'admins' && (
           <div className="space-y-6 animate-in fade-in duration-300">
              <div className="bg-white p-7 rounded-[40px] border shadow-sm space-y-5">
                <h3 className="text-xs font-black text-[#046A38] uppercase tracking-widest border-b pb-3 flex items-center space-x-2"><ShieldCheck size={20} /> <span>Tambah Tim Admin Baru</span></h3>
                <div className="space-y-4">
                  <div className="space-y-1"><label className="text-[10px] font-bold text-gray-400 uppercase ml-1">Nama Lengkap</label><input type="text" value={newAdminName} onChange={(e) => setNewAdminName(e.target.value)} className="w-full p-4 bg-gray-50 border rounded-2xl outline-none text-xs font-bold focus:border-[#046A38] shadow-inner" /></div>
                  <div className="space-y-1"><label className="text-[10px] font-bold text-gray-400 uppercase ml-1">WhatsApp (628...)</label><input type="tel" value={newAdminPhone} onChange={(e) => setNewAdminPhone(e.target.value)} className="w-full p-4 bg-gray-50 border rounded-2xl outline-none text-xs font-bold focus:border-[#046A38] shadow-inner" /></div>
                  <button onClick={handleAddAdmin} className="w-full py-5 bg-[#046A38] text-white font-black rounded-3xl text-[11px] tracking-widest uppercase shadow-lg shadow-emerald-900/20 active:scale-95 transition-all">Aktifkan Akses Admin</button>
                </div>
              </div>
              <div className="space-y-3 px-1">
                {adminList.map(adm => (<div key={adm.id} className="bg-white p-5 rounded-3xl border border-gray-150 flex items-center justify-between shadow-xs transition-all hover:border-[#046A38]"><div className="flex items-center space-x-4"><div className="w-12 h-12 bg-emerald-50 rounded-2xl flex items-center justify-center text-[#046A38] shadow-inner"><ShieldCheck size={24} /></div><div><p className="text-sm font-black text-gray-800 uppercase tracking-tight">{adm.nama}</p><p className="text-[10px] text-gray-500 font-bold mt-1">{adm.nomorHp} {adm.nomorHp === '6285156766317' && <span className="text-amber-600 ml-1">(SUPER)</span>}</p></div></div>{adm.nomorHp !== '6285156766317' && <button onClick={() => handleRemoveAdmin(adm.id)} className="p-2.5 text-red-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-all"><Trash2 size={20} /></button>}</div>))}
              </div>
           </div>
        )}
        ...
        {activeTab === 'logs' && (
           <div className="space-y-3 animate-in fade-in duration-300">
             ...
             <div className="space-y-2">{auditLogs.slice(0, 50).map(l => (<div key={l.id} className="bg-white p-5 rounded-[32px] border border-gray-150 space-y-2 shadow-xs hover:border-[#046A38] transition-all"><div className="flex justify-between items-start"><span className="text-[10px] font-black text-[#046A38] uppercase tracking-widest bg-emerald-50 px-3 py-1 rounded-full border border-emerald-100">{l.aksi}</span><p className="text-[9px] text-gray-400 font-mono">{new Date(l.timestamp).toLocaleString('id-ID')}</p></div><p className="text-[11px] text-gray-600 font-medium leading-relaxed">{l.detail}</p><div className="flex items-center space-x-2 pt-1 opacity-60"><User size={12} className="text-gray-400" /><p className="text-[9px] font-black text-gray-500 uppercase">Operator: {l.adminNama}</p></div></div>))}</div>
           </div>
        )}
      </div>
      ...
      {/* MODALS */}
      {showProofModal && (
        <div className="fixed inset-0 z-[2000] bg-black/95 flex flex-col items-center justify-center p-6"><button onClick={()=>setShowProofModal(null)} className="absolute top-8 right-8 text-white bg-white/10 p-3 rounded-full hover:bg-white/20 transition-all"><X size={28} /></button><div className="w-full max-w-sm aspect-[3/4] bg-white rounded-3xl overflow-hidden shadow-2xl ring-4 ring-white/10"><img src={showProofModal} className="w-full h-full object-contain" alt="Bukti Transfer Zoom" /></div><p className="text-white text-xs font-black mt-6 uppercase tracking-[0.4em] opacity-80">BUKTI TRANSFER RESMI</p></div>
      )}
      ...
      {showTopUpModal && (
        <div className="fixed inset-0 z-[1000] bg-black/70 backdrop-blur-md flex items-center justify-center p-6">
           <div className="bg-white w-full max-w-xs rounded-[40px] p-8 space-y-6 shadow-2xl animate-in zoom-in-95 duration-200 text-center">
              <div className="space-y-2"><div className="w-20 h-20 bg-emerald-50 rounded-full flex items-center justify-center mx-auto shadow-inner"><DollarSign size={40} className="text-emerald-600" /></div><h3 className="font-black text-base uppercase tracking-tight">Isi Saldo Mitra</h3><p className="text-[11px] text-gray-500 font-medium">Tambah saldo dompet driver secara instan.</p></div>
           ...
           <div className="bg-white w-full max-w-sm rounded-[40px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 flex flex-col max-h-[90vh] text-left">
              ...
              <div className="p-6 bg-gray-50 border-t flex space-x-3 shrink-0 shadow-inner">
                 {!selectedSopir.disetujuiAdmin ? (
                   ...
                   <button onClick={() => setShowSopirModal(false)} className="w-full py-5 bg-gray-900 text-white font-black rounded-3xl text-[11px] uppercase tracking-[0.2em] active:scale-95 transition-all">Tutup Detail</button>
                 )}
              </div>
           </div>
        </div>
      ...
      {showPanicOverlay && activeEmergency && (
        <div className="fixed inset-0 z-[1000] bg-red-600/90 backdrop-blur-xl flex items-center justify-center p-6 animate-pulse text-center">
           <div className="bg-white w-full max-w-sm rounded-[48px] shadow-2xl p-10 space-y-8 animate-in zoom-in-95 duration-500">
              ...
              <div className="space-y-2"><h2 className="text-3xl font-black text-red-600 uppercase tracking-tighter">DARURAT!</h2><p className="text-xs text-gray-500 font-bold uppercase tracking-[0.2em]">Sinyal SOS Diaktifkan Oleh:</p></div>
              ...
              <button onClick={() => { stopSiren(); setShowPanicOverlay(false); setActiveTab('darurat'); }} className="w-full py-6 bg-red-600 text-white font-black rounded-3xl text-xs tracking-[0.3em] uppercase shadow-2xl active:scale-95 transition-all border-b-8 border-red-900 shadow-red-900/40">SAYA TANGANI SEKARANG</button>
           </div>
        </div>
      )}
    </div>
