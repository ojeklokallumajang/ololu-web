...
import { ShieldAlert, AlertTriangle, Info, BellRing, Phone, ShieldCheck, UserPlus, LogIn, Camera, Check, ArrowRight } from 'lucide-react';
        ...
        <div className="fixed inset-0 bg-black/80 z-[9999] flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl border-4 border-[#DC2626] p-6 max-w-sm w-full text-center space-y-4 animate-bounce shadow-2xl">
            ...
            <div className="bg-red-50 p-3 rounded-2xl border border-red-200">
              <p className="text-[11px] text-[#DC2626] font-bold">⚠️ POSISI GPS AKURAT DAN INFORMASI KONTAK TELAH DIKIRIM KE SISTEM LOG RADAR ADMIN.</p>
            </div>
            <div className="space-y-2 pt-2">
              <button onClick={handleGotoAdminPanicRoom} className="w-full py-3 bg-[#DC2626] hover:bg-red-700 text-white font-black rounded-xl text-xs tracking-wider uppercase shadow-md transition-all">Buka Radar Penyelamatan Admin</button>
              <button onClick={() => setGlobalPanicNotification({ show: false, pelapor: '', tipe: '' })} className="w-full py-2 bg-gray-100 hover:bg-gray-200 text-gray-500 font-bold rounded-lg text-xs">Abaikan (Simulasi Selesai)</button>
            </div>
          </div>
        </div>
      )}
            ...
            {role === 'admin' ? (
              <div className="w-full flex flex-col xl:flex-row items-stretch justify-center gap-6">
                ...
                <div className="flex-1 bg-white shadow-xl rounded-none md:rounded-3xl border-none md:border md:border-gray-100 overflow-hidden">
                  <div className="p-4 md:p-6"><AdminView /></div>
                </div>
              </div>
            ) : (
