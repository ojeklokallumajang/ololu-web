  ...
  return (
    <div className="bg-white p-4 rounded-[32px] border border-gray-150 shadow-sm space-y-3">
      <div className="flex justify-between items-center px-1">
        <h3 className="text-sm font-black uppercase tracking-tight flex items-center space-x-2"><Compass size={16} className="text-emerald-600" /><span>Driver Sekitar</span></h3>
...
export default function PassengerView({ onNotifyAdminPanic, onLogout, onRoleChange, lockedOrderId }: PassengerViewProps) {
    ...
    <div className="max-w-md mx-auto bg-[#FAFBF9] min-h-screen pb-24 text-left">
      <div className="p-6 space-y-6">
        <div className="bg-[#034F2A] p-7 rounded-[40px] shadow-xl relative overflow-hidden text-white">
           ...
           <div className="relative z-10 flex justify-between items-start"><div><h3 className="text-xl font-black tracking-tight leading-none text-white">Halo, {profile?.nama}!</h3><p className="text-[10px] text-emerald-100/70 mt-2 font-bold uppercase tracking-widest leading-relaxed">Pilih layanan Ololu <br/> yang Anda butuhkan.</p></div><div className="w-12 h-12 bg-white/10 rounded-2xl flex items-center justify-center text-2xl shadow-inner">👋</div></div>
           {isSuperUser && <button onClick={() => onRoleChange('admin')} className="mt-6 w-full py-3 bg-[#D4AF37] text-[#034F2A] font-black rounded-2xl uppercase text-[9px] tracking-widest flex items-center justify-center space-x-2 shadow-lg"><ShieldCheck size={14} /><span>DASHBOARD ADMIN</span></button>}
        </div>
        ...
        <div className="grid grid-cols-4 gap-4 px-1">
           {[
             ...
             >
                <div className={`${serv.color} text-white w-14 h-14 rounded-2xl flex items-center justify-center shadow-lg group-active:scale-90 transition-transform`}>{serv.icon}</div>
                <span className="text-[10px] font-black text-gray-700 text-center leading-tight uppercase tracking-tighter">{serv.label}</span>
             </button>
           ))}
        </div>
        <LiveDriversMap config={config} />
      </div>
      <nav className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[420px] bg-white border-t border-gray-100 flex justify-around items-center h-20 z-50 shadow-[0_-10px_25px_rgba(0,0,0,0.05)] px-6"><button onClick={() => setViewMode('home')} className="flex flex-col items-center space-y-1 text-[#046A38]"><Home size={26} /><span className="text-[9px] font-black uppercase">Beranda</span></button><button onClick={() => setViewMode('history')} className="flex flex-col items-center space-y-1 text-gray-300"><History size={26} /><span className="text-[9px] font-black uppercase">Riwayat</span></button><button onClick={() => setViewMode('profile')} className="flex flex-col items-center space-y-1 text-gray-300"><User size={26} /><span className="text-[9px] font-black uppercase">Profil</span></button></nav>
    </div>
  );
  ...
  if (viewMode === 'history') return (
    <div className="max-w-md mx-auto bg-[#FAFBF9] min-h-screen pb-24 text-left">
       <div className="bg-[#034F2A] text-white p-5 border-b-2 border-[#D4AF37] shadow-xl sticky top-0 z-40 flex items-center space-x-3 text-white"><History size={20} className="text-[#D4AF37]" /><h1 className="text-base font-black uppercase tracking-widest text-white">Riwayat Perjalanan</h1></div>
       <div className="p-4 space-y-3">{historyOrders.map(p => (
          <div key={p.id} className="bg-white p-5 rounded-3xl border border-gray-150 shadow-sm space-y-3 text-gray-800"><div className="flex justify-between items-start"><div><div className="flex items-center space-x-2"><p className="text-sm font-black text-gray-800">#{p.nomorPesanan}</p><span className={`text-[8px] font-black px-2 py-0.5 rounded-lg text-white ${p.status === 'selesai' ? 'bg-emerald-500' : p.status === 'dibatalkan' ? 'bg-red-500' : 'bg-amber-400'}`}>{p.status.toUpperCase()}</span></div><p className="text-[10px] text-gray-400 font-bold mt-1">{new Date(p.waktuDibuat).toLocaleString('id-ID')}</p></div><p className="text-sm font-black text-[#046A38]">Rp {p.totalBayarAkhir?.toLocaleString()}</p></div><div className="text-[11px] text-gray-600 font-medium space-y-1.5 pt-2 border-t border-dashed"><p className="flex items-start space-x-2"><MapPin size={12} className="text-emerald-500 shrink-0 mt-0.5" /><span className="truncate">{p.asalAlamat}</span></p><p className="flex items-start space-x-2"><ArrowRight size={12} className="text-blue-500 shrink-0 mt-0.5" /><span className="truncate">{p.daftarTujuan[p.daftarTujuan.length-1].alamat}</span></p></div>{p.status === 'selesai' && <button onClick={() => generateReceipt(p)} className="w-full py-3 bg-gray-50 border-2 border-dashed border-gray-200 rounded-2xl text-[10px] font-black text-gray-500 uppercase tracking-widest flex items-center justify-center space-x-2 hover:text-emerald-600 transition-all"><FileText size={14} /><span>Download Nota</span></button>}</div>
       ))}</div>
       <nav className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[420px] bg-white border-t border-gray-100 flex justify-around items-center h-20 z-50 shadow-[0_-10px_25px_rgba(0,0,0,0.05)] px-6"><button onClick={() => setViewMode('home')} className="flex flex-col items-center space-y-1 text-gray-300"><Home size={24} /><span className="text-[9px] font-black uppercase">Beranda</span></button><button onClick={() => setViewMode('history')} className="flex flex-col items-center space-y-1 text-[#046A38]"><History size={24} /><span className="text-[9px] font-black uppercase">Riwayat</span></button><button onClick={() => setViewMode('profile')} className="flex flex-col items-center space-y-1 text-gray-300"><User size={24} /><span className="text-[9px] font-black uppercase">Profil</span></button></nav>
    </div>
  );
  ...
  if (viewMode === 'profile') return (
    <div className="max-w-md mx-auto bg-[#FAFBF9] min-h-screen pb-24 text-left">
       <div className="bg-[#034F2A] p-10 rounded-b-[60px] shadow-2xl relative overflow-hidden text-center text-white"><div className="w-28 h-28 bg-white/20 rounded-[40px] flex items-center justify-center mx-auto border-4 border-white/30 shadow-2xl mb-5 text-5xl text-white">👤</div><h2 className="text-2xl font-black tracking-tight text-white">{profile?.nama}</h2><p className="text-emerald-100/70 text-sm font-bold uppercase tracking-widest mt-1 text-white">{profile?.nomorHp}</p></div>
       <div className="p-8 space-y-6 text-gray-800"><div className="bg-white p-6 rounded-[40px] border border-gray-150 shadow-sm divide-y divide-gray-100"><div className="py-4 flex justify-between items-center"><span className="text-xs font-bold text-gray-400 uppercase tracking-widest">Total Trip</span><span className="text-sm font-black text-gray-700">{historyOrders.length} Selesai</span></div><div className="py-4 flex justify-between items-center"><span className="text-xs font-bold text-gray-400 uppercase tracking-widest">Status Akun</span><span className="text-[10px] font-black bg-emerald-100 text-emerald-700 px-3 py-1 rounded-full uppercase">Verified</span></div></div><button onClick={onLogout} className="w-full py-5 bg-red-50 text-red-600 font-black rounded-[32px] uppercase text-xs tracking-widest flex items-center justify-center space-x-3 active:scale-95 transition-all"><LogOut size={18} /><span>Keluar Aplikasi</span></button></div>
       <nav className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[420px] bg-white border-t border-gray-100 flex justify-around items-center h-20 z-50 shadow-[0_-10px_25px_rgba(0,0,0,0.05)] px-6"><button onClick={() => setViewMode('home')} className="flex flex-col items-center space-y-1 text-gray-300"><Home size={24} /><span className="text-[9px] font-black uppercase">Beranda</span></button><button onClick={() => setViewMode('history')} className="flex flex-col items-center space-y-1 text-gray-300"><History size={24} /><span className="text-[9px] font-black uppercase">Riwayat</span></button><button onClick={() => setViewMode('profile')} className="flex flex-col items-center space-y-1 text-[#046A38]"><User size={24} /><span className="text-[9px] font-black uppercase">Profil</span></button></nav>
    </div>
  );

  // --- RENDER: BOOKING (MULTI-STOP) ---
  const breakdown = getTarifBreakdown();
  return (
    <div className="max-w-md mx-auto bg-[#FAFBF9] min-h-screen text-left pb-24 text-gray-800">
      <div className="bg-[#034F2A] text-white p-5 rounded-b-[40px] border-b-2 border-[#D4AF37] flex justify-between items-center shadow-xl sticky top-0 z-40"><div className="flex items-center space-x-3 text-white"><div className="w-10 h-10 bg-white/10 rounded-xl flex items-center justify-center text-xl shadow-inner text-white">📍</div><h1 className="text-base font-black uppercase tracking-widest text-white">{subLayanan.toUpperCase()}</h1></div><button onClick={() => setViewMode('home')} className="bg-white/10 px-4 py-2 rounded-xl text-[10px] font-black tracking-widest text-white">BATAL</button></div>
      <div className="p-5 space-y-5 animate-in fade-in duration-300">
        <div className="bg-white p-6 rounded-[32px] border border-gray-150 shadow-sm space-y-6">
          ...
          <div className="space-y-4 pt-2 border-t-2 border-dashed"><div className="flex justify-between items-center ml-1"><label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Tujuan Pengantaran</label><span className="text-[9px] font-black text-gray-300 uppercase">{stops.length} Lokasi</span></div>
            {stops.map((s, i) => (<div key={s.id} className="flex space-x-2 animate-in slide-in-from-left-2 duration-300 text-gray-800"><button onClick={() => setMapPickerTarget(s.id)} className="flex-1 p-4 bg-gray-50 border-2 border-transparent hover:border-emerald-500 rounded-2xl text-left flex items-center space-x-4 transition-all min-w-0"><div className="bg-amber-100 p-2 rounded-xl text-amber-600 text-[10px] font-black w-8 h-8 flex items-center justify-center shadow-inner text-amber-600">{i + 1}</div><span className="text-xs font-bold text-gray-800 truncate">{s.alamat}</span></button>{stops.length > 1 && (<button onClick={() => handleRemoveStop(s.id)} className="p-4 bg-red-50 text-red-500 rounded-2xl shadow-sm text-red-500"><Trash2 size={20} /></button>)}</div>))}
            {stops.length < 5 && (<button onClick={handleAddStop} className="w-full py-4 border-2 border-dashed border-gray-200 rounded-2xl text-[10px] font-black text-gray-400 uppercase tracking-widest hover:border-emerald-500 hover:text-emerald-600 transition-all flex items-center justify-center space-x-2 bg-white text-gray-400"><Plus size={16} /><span>Tambah Perhentian</span></button>)}
          </div>
          <div className="pt-4 border-t-2 border-dashed space-y-3">
             <div className="space-y-1"><div className="flex justify-between text-[10px] font-black uppercase text-gray-400"><span>Tarif Perjalanan ({Math.ceil(routeDistance || 1)} KM)</span><span>Rp {(breakdown.total - breakdown.multi - breakdown.malam).toLocaleString()}</span></div>{breakdown.multi > 0 && <div className="flex justify-between text-[10px] font-black uppercase text-emerald-600"><span>Biaya Mampir ({stops.length - 1}x)</span><span>+ Rp {breakdown.multi.toLocaleString()}</span></div>}{breakdown.malam > 0 && <div className="flex justify-between text-[10px] font-black uppercase text-amber-600"><span>Surcharge Malam</span><span>+ Rp {breakdown.malam.toLocaleString()}</span></div>}</div>
             <div className="flex justify-between items-end"><div><span className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-none">Total Estimasi</span><p className="text-3xl font-black text-[#B8941F] tracking-tighter mt-1">Rp {breakdown.total.toLocaleString('id-ID')}</p></div><button onClick={()=>setPembayaranTunai(!pembayaranTunai)} className="bg-emerald-50 px-4 py-2 rounded-2xl border border-emerald-100 flex items-center space-x-2 active:scale-95 transition-all text-emerald-700 shadow-sm"><Tag size={12} className="text-emerald-600" /><span className="text-[10px] font-black text-emerald-700 uppercase tracking-widest">{pembayaranTunai ? '💵 TUNAI' : '📱 DOMPET'}</span></button></div>
          </div>
        </div>
